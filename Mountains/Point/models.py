...
#    objects = None
    ...
    status = models.CharField(max_length=20, choices = POSITION, default = NEW, unique=False)    #!!!Сделать так, чтобы доступ к этому полю был только у сотрудников ФСТР

    def __str__(self):
        return f'{self.name_pereval} {self.status}'
...
#    objects = None
    ...
    photo = models.ImageField(upload_to='photos/')                              # Определяем поле для вложений(фото)
    ...
    def __str__(self):
        return f'{self.pereval} {self.title} {self.description[:1000]} {self.coords} {self.level} {self.photo} ' \
               f'{self.user} {self.add_time}'
    # # Можно сделать так, чтобы в таблице Point поля pereval и level тоже отображались:
    # def __str__(self):
    #     perevals = ' '.join(str(pereval) for pereval in self.pereval.all())
    #     levels = ' '.join(str(level) for level in self.level.all())
    #     return f'{perevals}{self.title}{self.description[:1000]}{self.coords}{levels}{self.photo}{self.user}' \
    #            f'{self.add_time}'
